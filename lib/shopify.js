      ...
      products(first: 25) {
        edges {
          node {
            ...
            productSwatches: metafield(namespace: "customisation", key: "available_swatches") {
              value
              type
            }
          }
...
export async function getProduct(handle) { 
  ...
  const query = `
  {
    productByHandle(handle: "${handle}") {
      ...
      chairLifestyleImage: metafield(namespace: "image", key: "chair_lifestyle") {
        value
        type
      }
      ...
      bedLifestyleImage: metafield(namespace: "image", key: "bed_lifestyle") {
        value
        type
      }
      bedSingleImage: metafield(namespace: "image", key: "bed_single") {
        value
        type
      }
    }
...
// GET One Product based on parsed handle name : Used on product pages
export async function getProductsForRecommendations(productHandle) { 
  
  console.log(productHandle)

  const query = `
    {
      productByHandle(productHandle: ${productHandle}) {
        handle
        id
        productType
      }
...
//   const query = `
// {
//     productRecommendations(productId: "Z2lkOi8vc2hvcGlmeS9Qcm9kdWN0LzcyNjczNTQ3Mzg4Nzc=") {
//       id
//       title
//       handle
//       description
...
//  `
  ...
  const product = response.data.productRecommendations;
  console.log(response.data.productRecommendations)
